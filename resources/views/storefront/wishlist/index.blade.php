...
<div class="col-6 col-md-3"><a href="{{ route('products.show', $item->product->slug) }}" class="text-decoration-none"><div class="card product-card h-100"><div class="card-img-top d-flex align-items-center justify-content-center" style="height:180px;">@if($item->product->thumbnail)<img src="{{ asset('storage/'.$item->product->thumbnail) }}" class="w-100 h-100" style="object-fit:contain;">@else<i class="fas fa-box fa-3x text-muted opacity-25"></i>@endif</div><div class="card-body p-3"><div class="small text-muted mb-1">{{ $item->product->shop->name ?? '' }}</div><h6 class="small fw-semibold text-dark mb-2" style="display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">{{ $item->product->name }}</h6><span class="fw-bold text-primary">Rp {{ number_format($item->product->getEffectivePrice(),0,',','.') }}</span></div></div></a></div>
